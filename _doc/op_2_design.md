...
The remote HTML files are automatically re-generated when the remote repository is updated.
The remote repository is updated when it is synchronized with changes from the local repository.
...
If there is an error processing the HTML files in the remote site, an email notification will be sent regarding the failure.
